...
data,meta = parseFile("raw/PA01 Lactic Acid (1).xlsx",strain='PA01')
data2,meta2 = parseFile("raw/PAB Lactic Acid (1).xlsx",strain='PA1054')
...
data.columns = ['time'] + range(data.shape[1]-1)
data = data.iloc[4:,:]
data.to_csv("pseudomonas/data.csv",index=False)

meta.index = range(meta.shape[0])
meta.to_csv("pseudomonas/meta.csv",index=False)
#
